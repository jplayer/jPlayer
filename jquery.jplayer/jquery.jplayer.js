...
(function(jQuery) {
    ...
    // Adapted from jquery.ui.widget.js (1.8.7): jQuery.widget.bridge
    jQuery.fn.jPlayer = function( options ) {
        var name = "jPlayer";
        ...
        if ( isMethodCall ) {
            ...
            this.each(function() {
                ...
                if ( instance ) {
                    ...
                    jQuery.data( this, name, new jQuery.jPlayer( options, this ) );
                }
            });
        }
    ...
    jQuery.jPlayer = function( options, element ) {
    ...
    jQuery.jPlayer.event = {
        ready: "jPlayer_ready",
        resize: "jPlayer_resize", // Not implemented.
        error: "jPlayer_error", // Event error code in event.jPlayer.error.type. See jQuery.jPlayer.error
        warning: "jPlayer_warning", // Event warning code in event.jPlayer.warning.type. See jQuery.jPlayer.warning
    ...
    jQuery.jPlayer.htmlEvent = [ // These HTML events are bubbled through to the jPlayer event, without any internal action.
        "loadstart",
    ...
    jQuery.jPlayer.pause = function() {
        // jQuery.each(jQuery.jPlayer.instances, function(i, element) {
        jQuery.each(jQuery.jPlayer.prototype.instances, function(i, element) {
    ...
    jQuery.jPlayer.convertTime = function(sec) {
        var myTime = new Date(sec * 1000);
        var hour = myTime.getUTCHours();
        var min = myTime.getUTCMinutes();
        sec = sec || myTime.getUTCSeconds();
        var strHour = (jQuery.jPlayer.timeFormat.padHour && hour < 10) ? "0" + hour : hour;
        var strMin = (jQuery.jPlayer.timeFormat.padMin && min < 10) ? "0" + min : min;
        var strSec = (jQuery.jPlayer.timeFormat.padSec && sec < 10) ? "0" + sec : sec;
        return ((jQuery.jPlayer.timeFormat.showHour) ? strHour + jQuery.jPlayer.timeFormat.sepHour : "") + ((jQuery.jPlayer.timeFormat.showMin) ? strMin + jQuery.jPlayer.timeFormat.sepMin : "") + ((jQuery.jPlayer.timeFormat.showSec) ? strSec + jQuery.jPlayer.timeFormat.sepSec : "");
    };

    // Adapting jQuery 1.4.4 code for jQuery.browser. Required since jQuery 1.3.2 does not detect Chrome as webkit.
    jQuery.jPlayer.uaMatch = function( ua ) {
    ...
    jQuery.jPlayer.browser = {
    };

    var browserMatch = jQuery.jPlayer.uaMatch(navigator.userAgent);
    if ( browserMatch.browser ) {
        jQuery.jPlayer.browser[ browserMatch.browser ] = true;
        jQuery.jPlayer.browser.version = browserMatch.version;
    }

    jQuery.jPlayer.prototype = {
        ...
        options: { // Instanced in jQuery.jPlayer() constructor
...
*/
        ...
        _init: function() {
            ...
            if(!(this.html.used || this.flash.used)) {
                this._error( {
                    type: jQuery.jPlayer.error.NO_SOLUTION,
                    context: "{solution:'" + this.options.solution + "', supplied:'" + this.options.supplied + "'}",
                    message: jQuery.jPlayer.errorMsg.NO_SOLUTION,
                    hint: jQuery.jPlayer.errorHint.NO_SOLUTION
                });
            }
            ...
            if(this.flash.used) {
                ...
                if(jQuery.browser.msie && Number(jQuery.browser.version) <= 8) {
                    ...
                    var ie_dom = document.createElement(html_obj), i;
                    for(i=0; i < obj_param.length; i++) {
                ...
                this.internal.flash.jq = jQuery("#" + this.internal.flash.id);
            ...
            if(this.html.used && !this.flash.used) { // If only HTML, then emulate flash ready() call after 100ms.
                window.setTimeout( function() {
                    ...
                    self._trigger(jQuery.jPlayer.event.ready);
                }, 100);
            }
            ...
            jQuery.jPlayer.prototype.count++; // Change static variable via prototype.
        },
        destroy: function() {
            ...
            jQuery.each(this.css.jq, function(fn, jq) { // Remove any bindings from the interface controls.
                jq.unbind(".jPlayer");
            });
        ...
        _addHtmlEventListeners: function(mediaElement, entity) {
            ...
            mediaElement.addEventListener("progress", function() {
                if(entity.gate && !self.status.waitForLoad) {
                    ...
                    self._trigger(jQuery.jPlayer.event.progress);
                }
            }, false);
            mediaElement.addEventListener("timeupdate", function() {
                if(entity.gate && !self.status.waitForLoad) {
                    self._getHtmlStatus(mediaElement);
                    self._updateInterface();
                    self._trigger(jQuery.jPlayer.event.timeupdate);
                }
            }, false);
            mediaElement.addEventListener("durationchange", function() {
                if(entity.gate && !self.status.waitForLoad) {
                    ...
                    self._getHtmlStatus(mediaElement);
                    self._updateInterface();
                    self._trigger(jQuery.jPlayer.event.durationchange);
                }
            }, false);
            mediaElement.addEventListener("play", function() {
                if(entity.gate && !self.status.waitForLoad) {
                    self._updateButtons(true);
                    self._trigger(jQuery.jPlayer.event.play);
                }
            }, false);
            mediaElement.addEventListener("playing", function() {
                if(entity.gate && !self.status.waitForLoad) {
                    self._updateButtons(true);
                    self._seeked();
                    self._trigger(jQuery.jPlayer.event.playing);
                }
            }, false);
            mediaElement.addEventListener("pause", function() {
                if(entity.gate && !self.status.waitForLoad) {
                    self._updateButtons(false);
                    self._trigger(jQuery.jPlayer.event.pause);
                }
            }, false);
            mediaElement.addEventListener("waiting", function() {
                if(entity.gate && !self.status.waitForLoad) {
                    self._seeking();
                    self._trigger(jQuery.jPlayer.event.waiting);
                }
            }, false);
            mediaElement.addEventListener("canplay", function() {
                if(entity.gate && !self.status.waitForLoad) {
                    mediaElement.volume = self._volumeFix(self.options.volume);
                    self._trigger(jQuery.jPlayer.event.canplay);
                }
            }, false);
            mediaElement.addEventListener("seeking", function() {
                if(entity.gate && !self.status.waitForLoad) {
                    self._seeking();
                    self._trigger(jQuery.jPlayer.event.seeking);
                }
            }, false);
            mediaElement.addEventListener("seeked", function() {
                if(entity.gate && !self.status.waitForLoad) {
                    self._seeked();
                    self._trigger(jQuery.jPlayer.event.seeked);
                }
            }, false);
            mediaElement.addEventListener("suspend", function() { // Seems to be the only way of capturing that the iOS4 browser did not actually play the media from the page code. ie., It needs a user gesture.
                if(entity.gate && !self.status.waitForLoad) {
                    self._seeked();
                    self._trigger(jQuery.jPlayer.event.suspend);
                }
            }, false);
            mediaElement.addEventListener("ended", function() {
                if(entity.gate && !self.status.waitForLoad) {
                    ...
                    if(!jQuery.jPlayer.browser.webkit) { // Chrome crashes if you do this in conjunction with a setMedia command in an ended event handler. ie., The playlist demo.
                    ...
                    self._updateInterface();
                    self._trigger(jQuery.jPlayer.event.ended);
                }
            }, false);
            mediaElement.addEventListener("error", function() {
                if(entity.gate && !self.status.waitForLoad) {
                    ...
                    if(self.status.srcSet) { // Deals with case of clearMedia() causing an error event.
                        ...
                        self._error( {
                            type: jQuery.jPlayer.error.URL,
                            context: self.status.src, // this.src shows absolute urls. Want context to show the url given.
                            message: jQuery.jPlayer.errorMsg.URL,
                            hint: jQuery.jPlayer.errorHint.URL
                        });
                    }
                }
            }, false);
            // Create all the other event listeners that bubble up to a jPlayer event from html, without being used by jPlayer.
            jQuery.each(jQuery.jPlayer.htmlEvent, function(i, eventType) {
                mediaElement.addEventListener(this, function() {
                    if(entity.gate && !self.status.waitForLoad) {
                        self._trigger(jQuery.jPlayer.event[eventType]);
                    }
                }, false);
            });
        },
        ...
        _resetStatus: function() {
            this.status = jQuery.extend({}, this.status, jQuery.jPlayer.prototype.status); // Maintains the status properties that persist through a reset.
        },
        _trigger: function(eventType, error, warning) { // eventType always valid as called using jQuery.jPlayer.event.eventType
            var event = jQuery.Event(eventType);
            event.jPlayer = {};
            event.jPlayer.version = jQuery.extend({}, this.version);
            event.jPlayer.options = jQuery.extend(true, {}, this.options); // Deep copy
            event.jPlayer.status = jQuery.extend(true, {}, this.status); // Deep copy
            event.jPlayer.html = jQuery.extend(true, {}, this.html); // Deep copy
            event.jPlayer.flash = jQuery.extend(true, {}, this.flash); // Deep copy
            if(error){
                event.jPlayer.error = jQuery.extend({}, error);
            }
            if(warning){
                event.jPlayer.warning = jQuery.extend({}, warning);
            }
            this.element.trigger(event);
        },
        jPlayerFlashEvent: function(eventType, status) { // Called from Flash
            if(eventType === jQuery.jPlayer.event.ready && !this.internal.ready) {
                this.internal.ready = true;
                this.version.flash = status.version;
                if(this.version.needFlash !== this.version.flash) {
                    this._error( {
                        type: jQuery.jPlayer.error.VERSION,
                        context: this.version.flash,
                        message: jQuery.jPlayer.errorMsg.VERSION + this.version.flash,
                        hint: jQuery.jPlayer.errorHint.VERSION
                    });
                }
            ...
            if(this.flash.gate) {
                switch(eventType) {
                    case jQuery.jPlayer.event.progress:
                        this._getFlashStatus(status);
                        this._updateInterface();
                        this._trigger(eventType);
                        break;
                    case jQuery.jPlayer.event.timeupdate:
                        this._getFlashStatus(status);
                        this._updateInterface();
                        this._trigger(eventType);
                        break;
                    case jQuery.jPlayer.event.play:
                        this._seeked();
                        this._updateButtons(true);
                        this._trigger(eventType);
                        break;
                    case jQuery.jPlayer.event.pause:
                        this._updateButtons(false);
                        this._trigger(eventType);
                        break;
                    case jQuery.jPlayer.event.ended:
                        this._updateButtons(false);
                        this._trigger(eventType);
                        break;
                    case jQuery.jPlayer.event.error:
                        ...
                        break;
                    case jQuery.jPlayer.event.seeking:
                        this._seeking();
                        this._trigger(eventType);
                        break;
                    case jQuery.jPlayer.event.seeked:
                        this._seeked();
                        this._trigger(eventType);
                        break;
                    case jQuery.jPlayer.event.ready:
        ...
        _updateInterface: function() {
            ...
            if(this.css.jq.currentTime.length) {
                this.css.jq.currentTime.text(jQuery.jPlayer.convertTime(this.status.currentTime));
            }
            if(this.css.jq.duration.length) {
                this.css.jq.duration.text(jQuery.jPlayer.convertTime(this.status.duration));
            }
        },
        _seeking: function() {
            if(this.css.jq.seekBar.length) {
        ...
        setMedia: function(media) {
            ...
            if(supported) {
                ...
                // Send an error event
                this._error( {
                    type: jQuery.jPlayer.error.NO_SUPPORT,
                    context: "{supplied:'" + this.options.supplied + "'}",
                    message: jQuery.jPlayer.errorMsg.NO_SUPPORT,
                    hint: jQuery.jPlayer.errorHint.NO_SUPPORT
                });
            }
        },
        clearMedia: function() {
        ...
        _muted: function(muted) {
            ...
            this._updateVolume(this.options.volume);
            this._trigger(jQuery.jPlayer.event.volumechange);
        },
        ...
        _cssSelectorAncestor: function(ancestor) {
            ...
            if(ancestor && this.ancestorJq.length !== 1) { // So empty strings do not generate the warning.
                this._warning( {
                    type: jQuery.jPlayer.warning.CSS_SELECTOR_COUNT,
                    context: ancestor,
                    message: jQuery.jPlayer.warningMsg.CSS_SELECTOR_COUNT + this.ancestorJq.length + " found for cssSelectorAncestor.",
                    hint: jQuery.jPlayer.warningHint.CSS_SELECTOR_COUNT
                });
            }
            this._addUiClass();
        ...
        _cssSelector: function(fn, cssSel) {
            ...
            if(typeof cssSel === 'string') {
                if(jQuery.jPlayer.prototype.options.cssSelector[fn]) {
                    ...
                    if(cssSel && this.css.jq[fn].length !== 1) { // So empty strings do not generate the warning. ie., they just remove the old one.
                        this._warning( {
                            ...
                            message: jQuery.jPlayer.warningMsg.CSS_SELECTOR_COUNT + this.css.jq[fn].length + " found for " + fn + " method.",
                            hint: jQuery.jPlayer.warningHint.CSS_SELECTOR_COUNT
                        });
                    }
                } else {
                    this._warning( {
                        type: jQuery.jPlayer.warning.CSS_SELECTOR_METHOD,
                        context: fn,
                        message: jQuery.jPlayer.warningMsg.CSS_SELECTOR_METHOD,
                        hint: jQuery.jPlayer.warningHint.CSS_SELECTOR_METHOD
                    });
                }
            } else {
                this._warning( {
                    type: jQuery.jPlayer.warning.CSS_SELECTOR_STRING,
                    context: cssSel,
                    message: jQuery.jPlayer.warningMsg.CSS_SELECTOR_STRING,
                    hint: jQuery.jPlayer.warningHint.CSS_SELECTOR_STRING
                });
            }
        },
        ...
        option: function(key, value) {
            ...
            if(typeof key === "string") {
                ...
                if(value === undefined) {
                    ...
                    opt = jQuery.extend(true, {}, this.options);
                    for(i = 0; i < keys.length; i++) {
                        if(opt[keys[i]] !== undefined) {
                            opt = opt[keys[i]];
                        } else {
                            this._warning( {
                                type: jQuery.jPlayer.warning.OPTION_KEY,
                                context: key,
                                message: jQuery.jPlayer.warningMsg.OPTION_KEY,
                                hint: jQuery.jPlayer.warningHint.OPTION_KEY
                            });
                            return undefined;
                        }
                    }
        ...
        _setOptions: function(options) {
            var self = this;
            jQuery.each(options, function(key, value) { // This supports the 2 level depth that the options of jPlayer has. Would review if we ever need more depth.
        ...
        _html_initMedia: function() {
            ...
            this._trigger(jQuery.jPlayer.event.timeupdate); // The flash generates this event for its solution.
        },
        ...
        _html_clearMedia: function() {
            if(this.htmlElement.media) {
                ...
                if(!(jQuery.browser.msie && Number(jQuery.browser.version) >= 9)) { // IE9 Bug: media.load() on broken src causes an exception. In try/catch IE9 generates the error event too, but it is delayed and corrupts jPlayer's event masking.
        ...
        _checkForFlash: function (version) {
            ...
            if(window.ActiveXObject){
                ...
                if(flash){
                    var flashVersion = navigator.plugins["Shockwave Flash"].description.replace(/.*\s(\d+\.\d+).*/, "jQuery1");
                    if(flashVersion >= version){
                        flashIsInstalled = true;
                    }
                }
            }
            if(jQuery.browser.msie && Number(jQuery.browser.version) >= 9) { // IE9 does not work with external interface. With dynamic Flash insertion like jPlayer uses.
                return false;
            } else {
        ...
        _urlNotSetError: function(context) {
            this._error( {
                type: jQuery.jPlayer.error.URL_NOT_SET,
                context: context,
                message: jQuery.jPlayer.errorMsg.URL_NOT_SET,
                hint: jQuery.jPlayer.errorHint.URL_NOT_SET
            });
        },
        _flashError: function(error) {
            this._error( {
                type: jQuery.jPlayer.error.FLASH,
                context: this.internal.flash.swf,
                message: jQuery.jPlayer.errorMsg.FLASH + error.message,
                hint: jQuery.jPlayer.errorHint.FLASH
            });
        },
        _error: function(error) {
            this._trigger(jQuery.jPlayer.event.error, error);
            if(this.options.errorAlerts) {
                this._alert("Error!" + (error.message ? "\n\n" + error.message : "") + (error.hint ? "\n\n" + error.hint : "") + "\n\nContext: " + error.context);
            }
        },
        _warning: function(warning) {
            this._trigger(jQuery.jPlayer.event.warning, undefined, warning);
    ...
    jQuery.jPlayer.errorMsg = {
        FLASH: "jPlayer's Flash fallback is not configured correctly, or a command was issued before the jPlayer Ready event. Details: ", // Used in: _flashError()
        NO_SOLUTION: "No solution can be found by jPlayer in this browser. Neither HTML nor Flash can be used.", // Used in: _init()
        NO_SUPPORT: "It is not possible to play any media format provided in setMedia() on this browser using your current options.", // Used in: setMedia()
        URL: "Media URL could not be loaded.", // Used in: jPlayerFlashEvent() and _addHtmlEventListeners()
        URL_NOT_SET: "Attempt to issue media playback commands, while no media url is set.", // Used in: load(), play(), pause(), stop() and playHead()
        VERSION: "jPlayer " + jQuery.jPlayer.prototype.version.script + " needs Jplayer.swf version " + jQuery.jPlayer.prototype.version.needFlash + " but found " // Used in: jPlayerReady()
    };

    jQuery.jPlayer.errorHint = {
